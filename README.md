...
- MCP 工具助手：由管理员注册远程 Streamable HTTP 或 SSE MCP Server，模型可调用白名单内工具；前端以状态小卡片展示工具调用中、完成或失败
...
| 层级 | 技术 |
| --- | --- |
| 前端 | Next.js 16、React 19、TypeScript、Ant Design 6、Ant Design X、Tailwind CSS 4、Axios、Day.js |
...
│  │  ├─ repositories/       # 按领域拆分的数据访问：任务、文档、聊天、配置、用户
│  │  ├─ routers/            # AI、任务、文档、认证、MCP 和系统配置路由
│  │  ├─ services/           # 认证、配置、加密、邮件、上传策略、文档入库、任务队列和任务执行器
│  │  ├─ crud.py             # 兼容门面；新代码优先使用 app.repositories
...
│  │  ├─ chat/               # 对话页面及其局部组件、类型和工具函数
│  │  └─ tasks/              # 任务页面及其表单、历史抽屉、类型和工具函数
│  ├─ src/components/        # 全局布局等组件
│  ├─ src/lib/               # HTTP 客户端与认证工具
│  └─ package.json
├─ images/                   # README 页面示例截图
└─ docs/                     # 学习计划、代码笔记和权限隔离优化方案
```

后端代码组织说明：

- `app.main` 只负责应用装配，不再承载业务接口实现。
- 文档上传、下载、删除和进度 SSE 位于 `app.routers.documents`。
- 文档后台解析、状态推进和向量入库位于 `app.services.document_ingestion`。
- 文档处理邮件通知位于 `app.services.document_notifications`。
- 数据访问已经按领域拆到 `app.repositories.*`；`app.crud` 仅用于兼容旧调用。
...
| `AGENTCHAT_UPLOAD_MAX_BYTES` | `52428800` | 默认上传大小上限，单位字节 |
| `AGENTCHAT_UPLOAD_MAX_PDF_BYTES` | 跟随 `AGENTCHAT_UPLOAD_MAX_BYTES` | PDF 上传大小上限 |
| `AGENTCHAT_UPLOAD_MAX_DOCX_BYTES` | `31457280` | DOCX 上传大小上限 |
| `AGENTCHAT_UPLOAD_MAX_TEXT_BYTES` | `10485760` | TXT / Markdown 上传大小上限 |
| `AGENTCHAT_UPLOAD_CHUNK_READ_BYTES` | `1048576` | 上传文件流式读取块大小 |
| `AGENTCHAT_DOCUMENT_MAX_TEXT_CHARS` | `1000000` | 文档解析后允许入库的最大文本字符数 |
| `AGENTCHAT_DOCUMENT_MAX_CHUNKS` | `3000` | 单个文档允许生成的最大向量分片数 |
...
管理员登录后可以访问 `/mcp` 管理远程 Streamable HTTP 或 SSE MCP Server。页面支持：
...
1. 按扩展名读取上传策略并做大小限制
2. 流式保存原始文件并计算 SHA-256，创建 `uploaded_documents` 记录
3. 在 FastAPI 后台任务中解析和清洗文本
4. 以 600 字符、100 字符重叠切分文档
5. 生成 Embedding 并写入 Milvus
6. 通过 SSE 接口推送 `parsing`、`chunking`、`indexing` 等进度
7. 根据配置发送成功或失败邮件
...
| `GET` | `/documents/upload-policy` | 查询允许上传的扩展名、大小和解析限制 | 公开 |
...
如果 Windows 默认临时目录权限异常，可以把 pytest 临时目录放到项目内：

```powershell
uv run pytest tests -q --basetemp .test-tmp-pytest
```

本轮模块化相关的快速回归命令：

```powershell
uv run ruff check app/main.py app/crud.py app/repositories app/routers/documents.py app/services/document_ingestion.py app/services/document_notifications.py tests/test_upload.py tests/test_chat_memory.py
uv run pytest tests/test_tasks.py tests/test_upload.py tests/test_chat_memory.py tests/test_settings_secrets.py -q --basetemp .test-tmp-pytest-crud-split
```
...
如果 `npm run lint` 扫到本地包产物 `package/dist/`，应在 `eslint.config.mjs` 的 `globalIgnores` 中加入 `package/**` 或移除该本地产物目录，避免把外部打包文件作为项目源码检查。
